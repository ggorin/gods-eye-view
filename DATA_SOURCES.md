...
| **NYC DOT TMC (webcams.nyctmc.org)** | CCTV camera catalog + frames, New York City | Public NYC DOT webcams map API; see the note below on NYC DOT's data-sharing agreement | "NYC DOT Traffic Management Center — webcams.nyctmc.org" |
| **NYC Street Centerline (CSCL)** | Highway / bridge / tunnel / ramp centerlines with traffic direction, used to align NYC highway cameras onto the carriageway they face along | [NYC Open Data terms of use](https://opendata.cityofnewyork.us/overview/#termsofuse) — free for any use | "NYC Street Centerline — NYC Open Data" (courtesy) |
| **511NY (NYSDOT)** | CCTV camera catalog + still frames (NYC region by default, statewide opt-in), plus camera facings joined onto NYC DOT cameras | [511NY Developer's Access Agreement](https://511ny.org/developers/daa) — free, redistribution permitted, "powered by 511NY" credit encouraged | "powered by 511NY" |
...
- **NYC DOT cameras.** The catalog comes from the keyless `webcams.nyctmc.org/api/cameras`
  endpoint (the public webcams map's own API, ~975 cameras across the five boroughs) and
  frames from the per-camera `/api/cameras/<id>/image` still on the same origin. Only rows
  reporting `isOnline: "true"` with coordinates inside the city register; frames are fetched
  at request time through the proxy and never stored or bundled, and the credit is
  registered in the Data attribution popover. **Terms:** NYC DOT publishes no open-data
  license for this feed. Its webcams site links a "Video and Traffic Flow Data Sharing
  Partnership Agreement" for developers taking the cameras "via a data feed"; that
  agreement asks for attribution to the Department, permits access "via direct links to
  the Department's web services", and forbids transferring the contents of its web
  service directories to the user's own site. This pack follows those three conditions,
  but it has NOT been confirmed with NYC DOT that the public map API counts as a direct
  link rather than a feed requiring the signed agreement. `CCTV_NYCDOT_ENABLED=0` removes
  the pack entirely for deployments that prefer to wait for that confirmation.
  Two enrichments ride on the pack, each optional and self-catching: **511NY facings** —
  the `511ny.org/api/getcameras` list (NYSDOT; answers keyless today, `NY511_API_KEY` is
  appended when set, `CCTV_NYCDOT_511NY_ENABLED=0` skips it) is joined by coordinate within
...
- **511NY cameras.** The same `511ny.org/api/getcameras` list is also a pack of its own. The
  ...
  PNG snapshot on a 60 s cache, so that page is the frame and the stream is never touched.
  Enabled, unblocked rows inside the NYC box register by default (`CCTV_NY511_STATEWIDE=1`
  widens to the state, ~1,870 enabled cameras); rows within 30 m of an NYC DOT camera are
  dropped as the same physical mount, since the city still already carries the 511NY facing.
  `DirectionOfTravel` is a dedicated facing field, so a cardinal value sets the heading at
  high confidence. Cap 300 (`CCTV_NY511_MAX_SOURCES`), `CCTV_NY511_ENABLED=0` disables.
- **Road snap (NYC).** A signed facing ("Eastbound") names a carriageway, not a compass
  bearing — I-278 eastbound runs north through Brooklyn Heights. For NYC DOT and 511NY
  cameras with a real facing, the server fetches the NYC Street Centerline highway subset
  (`rw_type` highway/bridge/tunnel/ramp with `trafdir`, ~11k rows, ~3.7 MB, keyless; cached
  a week under `.gev-cache/cscl/`), finds carriageway pieces within 80 m whose direction of
  travel agrees with the signed facing (within 85°), and takes the nearest one's true
  bearing as the heading and its centerline point as the mount. Cameras with no agreeing
  road in range keep their prior. `CCTV_ROAD_SNAP_ENABLED=0` disables. Geometry is fetched
  at runtime and never bundled.
